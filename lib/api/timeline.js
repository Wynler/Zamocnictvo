...
export async function nacitajTimelineVyroby() {
  const { data: etapy, error: eErr } = await supabase
    .from('etapy')
    .select('id, nazov, clovekohod, pocet_ludi, datum_vyroby_od, datum_montaze_do, zinkovanie_dni, farba_dni, pieskovanie_dni, zakazky!inner(nazov, stav), dielce(mnozstvo, hmotnost_jedneho_ks)')
  ...
  for (const etapa of etapy) {
    ...
    for (const c of naplanovane) {
      riadky.push({
        ...
        etapaKoniecMontaze: etapa.datum_montaze_do,
        etapaPovrchDni,
        poradie: c.poradie,
