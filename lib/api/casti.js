 ...
 */
export async function vytvorCast(etapaId, nazov, poradie, priradenia, pocetLudi = null) {
  const { data: novaCast, error } = await supabase
    .from('casti_etapy')
    .insert([{ etapa_id: etapaId, nazov, poradie, pocet_ludi: pocetLudi }])
    .select()
    .single();
...
/**
 * Aktualizuje počet ľudí a/alebo ručný posun štartu na časti
 */
export async function aktualizujCast(castId, zmeny) {
  const payload = {};
  if ('pocetLudi' in zmeny) payload.pocet_ludi = zmeny.pocetLudi;
  if ('startOverride' in zmeny) payload.start_override = zmeny.startOverride;
