...
export function vypocitajVyrobnyPlan(etapa) {
  ...
  const koniecVyroby = pridajPracovneDni(startVyroby, trvanieVyroby);
  const zaciatokPovrchu = koniecVyroby;
  const koniecPovrchu = povrchDni > 0 ? pridajPracovneDni(zaciatokPovrchu, povrchDni) : zaciatokPovrchu;
  const zaciatokMontaze = koniecPovrchu;
  const koniecMontaze = montazDni > 0 ? pridajPracovneDni(zaciatokMontaze, montazDni) : zaciatokMontaze;
